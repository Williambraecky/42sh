...
/*   Updated: 2020/01/12 20:15:30 by wdaher-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// — !!
// — !word when word followd by smtg else "no such event"
// — !number when number out of range "no such event"
// — !-number
...
int		expand_exclamation(t_sh *shell, char *str, char **result)
{
  ...
  if (!str || str[0] != '!')
  ...
  else if (str[0] == '!' && ft_isalnum(str[1]) && ft_isalnum(str[2]))
    ...
    dprintf(2, "sh: event not found: %s\n", (str + 1));
  return (SH_SUCCESS);
}
